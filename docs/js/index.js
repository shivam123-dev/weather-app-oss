...
function generateWeatherQuote({
  ...
  const closers = ['Make it a good day.', 'Take it easy out there.', 'Keep it simple and steady.'];
...
async function generateGeminiQuote({
  apiKey,
  city,
  conditionGroup,
  weatherMain,
...
function renderSunTimes(weatherData) {
  ...
  const tryFallback = async () => {
    ...
    try {
      ...
      console.warn('Sunrise/sunset fallback failed', err);
    }
    return false;
  };
...
function renderWind(weatherData) {
  ...
  const updateLabels = () => {
    if (speedEl)
      speedEl.textContent =
        typeof speed === 'number' ? Math.round(speed) + ' ' + speedUnit : '-- ' + speedUnit;
    ...
    if (city) {
      (async () => {
        try {
          ...
          console.warn('Wind forecast fallback failed', err);
        }
      })();
    }
  }
